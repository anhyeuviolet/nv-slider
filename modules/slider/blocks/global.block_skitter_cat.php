...
if( ! nv_function_exists( 'nv_block_skitter' ) )
{
	function nv_block_config_skitter( $module, $data_block, $lang_block )
	{
	...
	function nv_block_config_skitter_submit( $module, $lang_block )
	{
		global $nv_Request;
		$return = array();
		$return['error'] = array();
	...
	function nv_block_skitter( $block_config )
	{
		...
		if( ! empty( $list ) )
		{
			if( file_exists( NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/slider/block_skitter.tpl' ) )
			{
			...
			$xtpl = new XTemplate( 'block_skitter.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/slider' );
...
if( defined( 'NV_SYSTEM' ) )
{
	...
	if( isset( $site_mods[$module] ) )
	{
		...
		$content = nv_block_skitter( $block_config );
	}
}
